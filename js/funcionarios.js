    ...
    $$('.toggle-btn', group).forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (explanationEl && explanationTexts) {
                explanationEl.innerHTML = explanationTexts[value];
            }
...
async function deleteFuncionario(id) {
    ...
    if (isInEscalaSalva) {
        showInfoModal({
            ...
            contentHTML: "<p>Este funcionário não pode ser excluído porque está registrado em uma ou mais escalas salvas. Para preservar o histórico, a exclusão não é permitida.</p>"
        });
        return;
    }
...
function initFuncionariosPage() {
    ...
    setupToggleGroup(contratoToggleGroup, funcContratoInput, contratoExplicacaoEl, {
        'clt': 'Funcionários <strong>CLT / Concursados</strong> seguirão rigorosamente as regras de descanso obrigatório.',
        'pj': 'Funcionários <strong>Prestadores de Serviço</strong> terão as regras de descanso ignoradas.'
    });
